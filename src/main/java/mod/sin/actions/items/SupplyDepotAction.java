...
public class SupplyDepotAction implements ModAction {
	private static final Logger logger = Logger.getLogger(SupplyDepotAction.class.getName());

	private final short actionId;
	private final ActionEntry actionEntry;

	public SupplyDepotAction() {
		logger.info("SupplyDepotAction()");
		...
		return new ActionPerformer() {
			...
			public boolean action(Action act, Creature performer, Item target, short action, float counter)
			{
				try{
					...
					return false;
				}catch(FailedException | NoSuchTemplateException | NoSuchActionException e){
