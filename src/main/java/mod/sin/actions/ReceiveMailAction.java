...
public class ReceiveMailAction implements ModAction {
	public static final Logger logger = Logger.getLogger(ReceiveMailAction.class.getName());

	private final short actionId;
	private final ActionEntry actionEntry;

	public ReceiveMailAction() {
		logger.info("ReceiveMailAction()");
				...
				if(performer instanceof Player){
					...
					if(!itemset.isEmpty()){
						...
						for (Item item : itemset) {
							...
							for (Item c4 : contained4) {
								c4.setMailed(false);
								c4.setLastOwnerId(performer.getWurmId());
							}
