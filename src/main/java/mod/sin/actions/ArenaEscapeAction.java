...
public class ArenaEscapeAction implements ModAction {
	public static final Logger logger = Logger.getLogger(ArenaEscapeAction.class.getName());

	private final short actionId;
	private final ActionEntry actionEntry;

	public ArenaEscapeAction() {
		logger.info("ArenaEscapeAction()");
				...
				try{
					if(performer instanceof Player){
						...
						if(performer.isStealth()){
							performer.getCommunicator().sendNormalServerMessage("You cannot escape while in stealth mode.");
							return true;
						}
						...
						if(counter == 1.0f){
							...
							int tilex = 1010;
							int tiley = 1010;
							performer.sendTransfer(Server.getInstance(), targetserver.INTRASERVERADDRESS, Integer.parseInt(targetserver.INTRASERVERPORT), targetserver.INTRASERVERPASSWORD, targetserver.id, tilex, tiley, true, false, performer.getKingdomId());
