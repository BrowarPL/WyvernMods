...
public class ArrowPackWar {
	public static final Logger logger = Logger.getLogger(ArrowPackWar.class.getName());
	public static int templateId;
	private static final String NAME = "war arrow pack";
	...
	public void initCreationEntry() {
		...
		if (templateId > 0) {
			try {
				AdvancedCreationEntry warPack = CreationEntryCreator.createAdvancedEntry(SkillList.GROUP_FLETCHING,
						ItemList.quiver, ItemList.arrowWar, templateId,
						false, false, 0.0F, true, false, 0, 50.0D, CreationCategories.FLETCHING);
				if (warPack != null) {
					warPack.addRequirement(new CreationRequirement(1, ItemList.arrowWar, 39, true));
				} else {
