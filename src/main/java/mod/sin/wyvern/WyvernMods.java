...
public class WyvernMods implements
		WurmServerMod, Configurable, PreInitable,
		Initable, Versioned, ItemTemplatesCreatedListener,
		ServerStartedListener, ServerPollListener,
		PlayerLoginListener, ChannelMessageListener, ModListener {
	public static final Logger logger = Logger.getLogger(WyvernMods.class.getName());
	public static final String version = "ty3.1.2";

	// Tyoda stuff
	public static boolean useGlimmerscaleModel = false;
	public static boolean useCustomIcons = false;
	public static boolean useKnucklesIcon = true;
	public static boolean useGlimmerscaleIcons = true;

	public static boolean craftSpectralSet = true;

	public static boolean craftGlimmerscaleSet = true;
	...
	public static boolean rareCreationAdjustments = false;
	...
	public void configure(Properties properties) {
		...
		// Tyoda stuff
		useGlimmerscaleModel = Prop.getBooleanProperty("useGlimmerscaleModel", useGlimmerscaleModel);
		useCustomIcons = Prop.getBooleanProperty("useCustomIcons", useCustomIcons);
		useKnucklesIcon = Prop.getBooleanProperty("useKnucklesIcon", useKnucklesIcon);
		useGlimmerscaleIcons = Prop.getBooleanProperty("useGlimmerscaleIcons", useGlimmerscaleIcons);
		craftSpectralSet = Prop.getBooleanProperty("craftSpectralSet", craftSpectralSet);
		craftGlimmerscaleSet = Prop.getBooleanProperty("craftGlimmerscaleSet", craftGlimmerscaleSet);
				...
				switch (name) {
					...
					default:
						if (name.startsWith("infoTabLine")) {
							...
							int skillId = Integer.parseInt(values[3]);
	...
	public void onItemTemplatesCreated() {
		if(WyvernMods.useCustomIcons){
			IconzzHandler.onItemTemplatesCreated();
		}
	...
	public void onServerStarted() {
		...
		try {
			...
			if (WyvernMods.useGlimmerscaleModel){
				ServerPackHandler.onServerStarted();
			}
	...
	public void modInitialized(ModEntry<?> modEntry) {
		if (WyvernMods.useGlimmerscaleModel){
			ServerPackHandler.modInitialized(modEntry);
		}

		if (WyvernMods.useCustomIcons){
			IconzzHandler.modInitialized(modEntry);
		}
	}
