...
public class Arena {
    ...
    public static void createNewHotaPrize(Village v, int winStreak){
        try {
            ...
            // 5-8 caches
            for(int i = 5+Server.rand.nextInt(4); i > 0; --i) {
            ...
            for(int i = 10+Server.rand.nextInt(11); i > 0; --i){
            ...
            for(int i = 4+Server.rand.nextInt(3); i > 0; --i){
    ...
    public static void preInit(){
        try {
            ...
            if (WyvernMods.makeFreedomFavoredKingdom) {
                ...
                Util.setReason("Set favored kingdom for PvP");
                replace = "{ return (byte) 4; }";
                Util.setBodyDeclared(thisClass, ctDeity, "getFavoredKingdom", replace);
            }
