...
public class MissionCreator {
    public static final Logger logger = Logger.getLogger(MissionCreator.class.getName());
    ...
    public static void pollMissions(){
            ...
            if (mission.isCurrent()) {
                if (mission.isCompleted() || mission.getEndTime() <= System.currentTimeMillis()) {
                    try {
                        ...
                        logger.log(Level.WARNING, "Failed in MissionCreator", e);
                    }
                }
            }
        }
        ...
        while(++tries <= 10) {
            ...
            if(EpicServerStatus.getEpicMissionForEntity(candidate) == null){
                number = candidate;
                logger.info("Entity " + number + " has no mission, beginning creation.");
                break;
            }else{
                logger.info("Entity " + candidate + " has a mission, finding new entity.");
            }
        }
        ...
        if (EpicServerStatus.getCurrentScenario() != null) {
            epicServerStatus.generateNewMissionForEpicEntity(
                    number,
                    entityName,
                    -1,
                    time,
