...
public class Mastercraft {
	public static final Logger logger = Logger.getLogger(Mastercraft.class.getName());
	...
	public static double getNewDifficulty(Skill skill, double diff, Item item){
		...
		if(item != null){
			...
			if(WyvernMods.legendItemDifficultyBonus && item.getCurrentQualityLevel() > 99.0f){
				diff -= 1d - (100d - item.getCurrentQualityLevel());
			}
	...
	private static void instrumentSpellRuns(CtMethod[] methods) throws CannotCompileException {
