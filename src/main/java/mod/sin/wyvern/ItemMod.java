...
public class ItemMod {
	public static final Logger logger = Logger.getLogger(ItemMod.class.getName());
	...
	public static void initCreationEntries(){
		...
		// Spectral set
		if(WyvernMods.craftSpectralSet){
		...
		// Glimmerscale set
		if(WyvernMods.craftGlimmerscaleSet) {
	...
	public static void onServerStarted(){
		if (WyvernMods.removeLockpickSkillRequirement) {
			...
			try {
				...
				logger.log(Level.WARNING, "Failed to set lockpick creation entry changes!", e);
			}
		}
	}
		...
		try {
			...
			HookManager.getInstance().registerHook("com.wurmonline.server.behaviours.VehicleBehaviour", "getVehicleBehaviours",
					Descriptor.ofMethod(output, input), () -> (proxy, method, args) -> {
						...
						LinkedList<ActionEntry> permissions = new LinkedList<>();
