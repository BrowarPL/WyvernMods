    ...
    public void spawn(Player p, Spawnpoint spawnpoint) {
        if (p.isDead()) {
            ...
            try { //p.setDead(false);
                ...
                WyvernMods.logger.log(Level.WARNING, "Failed in NewSpawnQuestion", e);
            }
            p.spawnpoints = null;
        }
    }
        ...
        if (accepted) {
            ...
            if(transfer) {
                ...
                try {
                    ...
                    WyvernMods.logger.log(Level.WARNING, "Could not find player for WurmId " + this.getResponder().getWurmId() + " [" + this.getResponder().getName() + "]", e);
                }
            }else {
